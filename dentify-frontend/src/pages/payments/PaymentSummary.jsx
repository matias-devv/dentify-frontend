...
const formatWeekday = (dayString) => {
  ...
  return map[dayString?.toUpperCase()] || dayString;
};
...
export default function PaymentSummary() {
  ...
  // Derivados correctos — solo a partir de lo que retorna el backend
    ...
    <div className="bg-[#F4F3F0] text-on-surface font-body-md min-h-screen">
      <main className="px-8 py-8 pb-24 w-full">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 mt-[32px]">
                  ...
                  {topDebtors.length > 0 ? (
                    topDebtors.map((debtor, idx) => (
                      <tr key={idx} className="border-b border-outline-variant/10 hover:bg-[#F8F8F6] transition-colors">
                        ...
                        <td className="p-5 text-right font-data-mono text-data-mono text-[#4A9EE8] font-bold">{formatCurrency(debtor.total_debt)}</td>
                        <td className="p-5 text-center">{debtor.pending_payments_count}</td>
                      </tr>
                    ))
                  ) : (
                    <tr>
                      <td colSpan={4} className="p-5 text-center text-on-surface-variant">Sin deudores registrados</td>
                    </tr>
                  )}
                </tbody>
              </table>
          ...
          {/* ============================================================
              ESTADO DE PAGOS DEL MES — rediseñado
              Datos reales del backend:
                - collectionRate.confirmedPayments  (ej: 9)
                - collectionRate.pendingPayments    (ej: 8)
                - collectionRate.totalPaymentsMonth (ej: 17)
                - collectionRate.ratePercentage     (ej: 52.9)
              NO se usan montos calculados a partir de monthly_income.
          ============================================================ */}
          <div className="lg:col-span-5 bg-white rounded-xl p-6 custom-shadow border border-outline-variant/10 flex flex-col">
            {/* Header */}
            <div className="border-b border-outline-variant/20 pb-4 mb-6">
            ...
            <div className="flex-1 flex flex-col items-center gap-6">
              {/* Donut — sin texto central, solo el anillo bicolor */}
      ...
      <style>{`
        ...
        .navy-gradient {
          background: linear-gradient(180deg, #101c2e 0%, #001b3e 100%);
        }
        .custom-shadow {
          box-shadow: 0 2px 20px rgba(10, 22, 40, 0.08);
        }
